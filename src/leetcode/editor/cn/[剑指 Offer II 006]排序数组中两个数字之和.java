...
//array-is-sorted/ 
// Related Topics 数组 双指针 二分查找 👍 38 👎 0
...
class Solution {
    public int[] twoSum(int[] numbers, int target) {
        ...
        for (int i = 0; i < numbers.length - 1 && numbers[i] <= target; ++i) {
            int second = binarySearch(numbers, target - numbers[i], i + 1);
            if (second != -1) {
                res[0] = i;
                res[1] = second;
                return res;
    ...
    int binarySearch(int[] numbers, int target, int lBound) {
        int left = lBound, right = numbers.length - 1;
