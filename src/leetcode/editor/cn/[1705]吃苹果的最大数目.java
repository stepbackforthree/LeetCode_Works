...
class Solution {
    public int eatenApples(int[] apples, int[] days) {
        ...
        int currDay = 1;
        int result = 0;
        while (!queue.isEmpty() || currDay <= apples.length) {
            if (currDay <= apples.length) {
                queue.add(new int[]{apples[currDay - 1], currDay + days[currDay - 1]});
            }

            while (!queue.isEmpty() && queue.peek()[1] <= currDay) {
                queue.poll();
            }
            if (queue.isEmpty() && currDay > apples.length) {
                break;
            }
